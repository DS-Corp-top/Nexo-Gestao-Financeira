...
async function completeResetFlow(
  ...
  await Promise.all([
    page.waitForResponse((response) =>
      response.url().includes('/api/v1/auth/password-reset/request/') && response.ok()
    ),
    page.getByRole('button', { name: 'Enviar código' }).click(),
  ]);
  await expect(page.getByLabel('Código de confirmação')).toBeVisible({ timeout: 15_000 });
  ...
  await Promise.all([
    page.waitForResponse((response) =>
      response.url().includes('/api/v1/auth/password-reset/confirm/') && response.ok()
    ),
    page.getByRole('button', { name: 'Confirmar código' }).click(),
  ]);
  await expect(page.getByLabel('Nova senha', { exact: true })).toBeVisible({ timeout: 15_000 });

  await page.getByLabel('Nova senha', { exact: true }).fill(newPassword);
  await page.getByLabel('Confirmar nova senha').fill(newPassword);
  await Promise.all([
    page.waitForResponse((response) =>
      response.url().includes('/api/v1/auth/password-reset/complete/') && response.ok()
    ),
    page.getByRole('button', { name: 'Redefinir senha' }).click(),
  ]);

  await expect(page.getByText('Sua senha foi alterada.')).toBeVisible({ timeout: 15_000 });
}
...
test('recupera a senha por e-mail e consegue logar com a nova senha', async ({ page, request }) => {
  ...
  // com E2E_USER_PASSWORD, e o seed so roda uma vez no inicio da suite.
