...
export async function toggleTransactionCleared({ id, cleared_date, unlock_password, amount, account, category, description }: { id: number, cleared_date?: string, unlock_password?: string, amount?: string, account?: number, category?: number | null, description?: string }): Promise<Transaction> {
  const { data } = await api.post<Transaction>(`/transactions/${id}/toggle_cleared/`, { cleared_date, unlock_password, amount, account, category, description });
  return data;
}
